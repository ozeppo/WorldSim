...
local function parseArgs(raw)
    local options = {
        out = "agent-ai/nation_real_states.csv",
...
local function resourcePressure(sim, agent)
    local x = agent.homeX or agent.x
    local y = agent.homeY or agent.y
    return sim.world:resourcePressureAround(x, y, 5)
end
...
local function collect(sim, file)
    ...
    for _, agent in ipairs(sim.agents) do
        if agent.alive and agent.communityId and (agent.prosperity or 0) > 30 then
            local row = NationAI.featureRow(agent, sim, {
                localResources = resourcePressure(sim, agent)
            })
            if row then
                writeRow(file, row)
                written = written + 1
...
out:write(table.concat(NationAI.features, ","), "\n")

local total = 0
local started = os.clock()
for run = 1, options.runs do
    local seed = (options.seed or 1) + run * 9973
    local sim = makeSimulation(config, options, seed)
    local runRows = 0
    for _ = 1, options.ticks do
        sim:step()
        if sim.tick % options.sample_every == 0 then
            runRows = runRows + collect(sim, out)
        end
    end
    total = total + runRows
    io.stderr:write(string.format("run=%d seed=%d rows=%d total=%d tick=%d people=%d settlements=%d nations=%d\n", run, seed, runRows, total, sim.tick, #sim.agents, sim:communityCount(), sim.nationCount and sim:nationCount() or 0))
end
